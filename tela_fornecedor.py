...
class CRUDApp:
    ...
    def create_widgets(self):
        ...
        tk.Label(self.root,text="Id Fornecedor (for update/delete):").grid(row=3,column=0)
        ...
        self.idfornecedor_entry.grid(row=3,column=1)
        #botões do crud
        tk.Button(self.root,text="Adicionar fornecedor",command=self.add_supplier).grid(row=5,column=0,columnspan=1)
        tk.Button(self.root,text="Listar fornecedores",command=self.read_suppliers).grid(row=5,column=1,columnspan=1)
        tk.Button(self.root,text="Alterar fornecedores",command=self.update_supplier).grid(row=6,column=0,columnspan=1)
        tk.Button(self.root,text="Excluir fornecedores",command=self.delete_supplier).grid(row=6,column=1,columnspan=1)
    ...
    def add_supplier(self):
        ...
        if nome and produtofornecido and quantiamensal:
            add_supplier(nome,produtofornecido,quantiamensal)
    ...
    def read_suppliers(self):
        ...
        for supplier in suppliers:
            self.text_area.insert(tk.END,f"id: {supplier[0]}, nome: {supplier[1]}, produto_rnecido: {supplier[2]}, quantia mensal: {supplier[3]}\n")
    ...
    def delete_supplier(self):
        idsupplier=self.idfornecedor_entry.get()
        if idsupplier:
            delete_supplier(idsupplier)
            self.idfornecedor_entry.delete(0,tk.END)
